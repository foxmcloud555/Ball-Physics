...
HeightMap::HeightMap(char* filename, float gridSize, float heightRange)
{
	...
	for (int f = 0; f < m_HeightMapFaceCount; ++f)
	{
		...
		m_pFaceData[f].m_bSelected = false;
	}
	...
	for (int l = 0; l < m_HeightMapLength; ++l)
	{
		for (int w = 0; w < m_HeightMapWidth; ++w)
		{
			if (w < m_HeightMapWidth - 1 && l < m_HeightMapLength - 1)
			{
				...
				XMStoreFloat3(&m_pFaceData[faceIndex + 0].m_D, -XMVector3Dot(vN1, v0));
				...
				XMStoreFloat3(&m_pFaceData[faceIndex + 1].m_D, -XMVector3Dot(vN2, v2));
...
void HeightMap::RebuildVertexData(void)
{
	...
	if (SUCCEEDED(Application::s_pApp->GetDeviceContext()->Map(m_pHeightMapBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &map)))
	{
		...
		static VertexColour STANDARD_COLOUR(200, 200, 200, 255);
		static VertexColour COLLISION_COLOUR(255, 0, 0, 255);
		static VertexColour HIGHLIGHT_COLOUR(255, 0, 255, 255); // PURPLE!!!
		...
		for (int f = 0; f < m_HeightMapFaceCount; f += 2)
		{
			...
			/*if (m_pFaceData[f + 0].m_bCollided)
				c0 = COLLISION_COLOUR;
			if (m_pFaceData[f + 1].m_bCollided)
				c1 = COLLISION_COLOUR;

			if (m_pFaceData[f + 0].m_bSelected)
				c0 = HIGHLIGHT_COLOUR;
			if (m_pFaceData[f + 1].m_bSelected)
				c1 = HIGHLIGHT_COLOUR;*/
...
bool HeightMap::RayCollision(XMVECTOR& rayPos, XMVECTOR rayDir, float raySpeed, XMVECTOR& colPos, XMVECTOR& colNormN, XMFLOAT3& faceNorm)
{
	...
	for (int f = 0; f < m_HeightMapFaceCount; ++f)
	{
		...
		if (!m_pFaceData[f].m_bDisabled && RayTriangle(f, rayPos, rayDir, colPos, colNormN, colDist))
		{
			...
			if (colDist <= raySpeed && colDist >= 0.0f)
			{
				m_pFaceData[f].m_bCollided = true;
				RebuildVertexData();
				faceNorm = m_pFaceData[f].m_vNormal;
				return true;
			}
		}
	}
...
bool HeightMap::RayTriangle(int nFaceIndex, const XMVECTOR& rayPos, const XMVECTOR& rayDir, XMVECTOR& colPos, XMVECTOR& colNormN, float& colDist)
{
	...
	// D = Z on presentation slides
	// -D = Ax + By + Cz
	// A, B, C are the x, y, z of the normal |COLNORM| 
	// x, y, z are x, y, z of a point on the plane 
	// Equivalent to -D = colNormN . vert0
	XMFLOAT3 D = m_pFaceData[nFaceIndex].m_D;

	// Step 3: Calculate the demoninator of the COLDIST equation: (|COLNORM| dot |RAYDIR|) and early out if it is 0
	XMVECTOR rayDirN = XMVector3Normalize(rayDir);
	XMVECTOR demoninator = XMVector3Dot(colNormN, rayDirN);
	if (XMVectorGetX(demoninator) == 0)
		return false;

	// Step 4: Calculate the numerator of the COLDIST equation: -(D+(|COLNORM| dot RAYPOS))
	XMVECTOR numerator = (XMLoadFloat3(&D) + (XMVector3Dot(colNormN, rayPos)));

	// Step 5: Calculate COLDIST and early out if COLDIST is behind RAYDIR
	colDist = XMVectorGetX(-numerator / demoninator);
	if (colDist < 0)
		return false;

	// Step 6: Use COLDIST to calculate COLPOS
	colPos = (rayPos + (colDist * rayDirN));
...
bool HeightMap::PointPlane(const XMVECTOR& vert0, const XMVECTOR& vert1, const XMVECTOR& pointPos, const XMVECTOR& rayPos)
{
	...
	XMVECTOR edge1 = (vert0 - rayPos);
	XMVECTOR edge2 = (vert1 - rayPos);
	XMVECTOR planeNormal = XMVector3Cross(edge1, edge2);

	// Step 2: Calculate D
	//-D = planeNorm . newRayPos
	XMVECTOR planeD = -XMVector3Dot(planeNormal, rayPos);

	// Step 3: Calculate full equation
	// Step 4: Return false if < 0 (behind plane)

	// planeNorm . newRayPos + D > 0
	if (XMVectorGetX((XMVector3Dot(planeNormal, pointPos) + (planeD))) > 0) {
		return false;
	}
